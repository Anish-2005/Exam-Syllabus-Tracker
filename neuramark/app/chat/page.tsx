...
export default function ChatPage() {
                ...
                for (const memberId of currentRoom.members || []) {
                    ...
                    if (userDoc.exists()) {
                        ...
                        memberDetails.push({
                            ...
                            role: currentRoom.admin === memberId
                                ? "admin"
    ...
    const createNewRoom = async () => {
        ...
        try {
            ...
            setCurrentRoom({
                ...
                admin: newRoom.admin,
                moderators: newRoom.moderators,
                createdAt: new Date(),
                updatedAt: new Date(),
                isPrivate: newRoom.type === 'private',
                isGlobal: false,
                type: newRoom.type as 'public' | 'private' | 'global',
                code: newRoom.code,
            })
    ...
    const handleJoinRequest = async (requestId: string, action: string) => {
        try {
    ...
    const makeUserModerator = async (userId: string) => {
    ...
    const removeUserModerator = async (userId: string) => {
        if (!currentRoom || !canManageRoles()) return
    ...
    const getUserRole = () => {
        ...
        if (user?.uid && currentRoom.moderators?.includes(user.uid)) return "moderator"
        return "member"
    }
